"""BHX Discovery: test ALL slugs, collect unique products"""
import requests
import json
import time
import sys
...
def extract(items, parent=""):
    for item in items:
        ...
        if slug and slug != "he-thong-sieu-thi" and not slug.startswith("thuong-hieu"):
            all_slugs[slug] = {"name": name, "parent": parent, "total": 0, "returned": 0}
        for child in item.get("childrens", []):
            ...
            if c_url:
                all_slugs[c_url] = {"name": c_name, "parent": name, "total": 0, "returned": 0}
            for sub in child.get("childrens", []):
                s_params = sub.get("appScreen", {}).get("parameters", {})
                s_url = s_params.get("categoryUrl", "") or sub.get("url", "")
                if s_url:
                    all_slugs[s_url] = {"name": sub.get("name", ""), "parent": c_name, "total": 0, "returned": 0}
...
for i, (slug, info) in enumerate(sorted(all_slugs.items())):
    try:
        ...
        if returned > 0:
            slug_stats.append({"slug": slug, "name": info["name"], "total": total, "returned": returned, "new": len([p for p in prods if p["id"] not in all_products])})
...
print(f"\nTop slugs by total:")
slug_stats.sort(key=lambda x: x["total"], reverse=True)
for s in slug_stats[:30]:
    print(f"  {s['slug']:35s} total={s['total']:4d} returned={s['returned']:2d}")

# Save all products
out = "data/bhx_discovered.json"
