...
const CATEGORY_EMOJIS = {
  ...
  pantry: "🍚", condiments: "🧂", dairy: "🥛", beverages: "🥤"
};
    ...
    <aside class="catalog-sidebar" id="catalog-sidebar">
      ...
      <div class="filter-group">
        ...
        <div class="filter-group__options">
          <label class="filter-radio ${!state.category ? 'is-active' : ''}">
            <input type="radio" name="category" value="" ${!state.category ? 'checked' : ''} /> Tất cả
          </label>
          ${state.categories.map(c => `
            <label class="filter-radio ${state.category === c.id ? 'is-active' : ''}">
              <input type="radio" name="category" value="${c.id}" ${state.category === c.id ? 'checked' : ''} />
              ${CATEGORY_EMOJIS[c.id] || "📦"} ${escapeHTML(c.name)}
            </label>
          `).join("")}
        </div>
      </div>
      ...
      <div class="filter-group">
        ...
        <div class="filter-group__options">
          <label class="filter-checkbox">
            <input type="checkbox" id="filter-sale" ${state.onlySale ? 'checked' : ''} /> 🏷️ Đang giảm giá
          </label>
          <label class="filter-checkbox">
...
function renderFilterTags() {
  ...
  if (state.category) tags.push(`<span class="filter-tag">${CATEGORY_EMOJIS[state.category]||"📦"} ${escapeHTML(getCategoryName(state.category))} <span class="filter-tag__remove" data-clear="category">✕</span></span>`);
  if (state.onlySale) tags.push(`<span class="filter-tag">🏷️ Đang sale <span class="filter-tag__remove" data-clear="onlySale">✕</span></span>`);
...
async function initCatalogPage() {
  ...
  state.category = getQueryParam("category") || saved.category || "";
